...
if __name__ == "__main__":
    ...
    AUG_XML_DIR = "./aug_Annotantions/"  # 存储增强后的XML文件夹路径
    try:
    ...
    # 影像增强
    seq = iaa.Sequential([
        #iaa.GaussianBlur(sigma=(2.0, 5.0)),  # 高斯模糊
        #iaa.MotionBlur(k=(7,8 )),  # 运动模糊
        #iaa.Sharpen(alpha=(0.25, 1.00), lightness=(0.3, 0.5)),  # 锐化图像
        #iaa.MultiplyBrightness((1.2, 1.4)), # 增加亮度
        #iaa.MultiplyBrightness((0.5, 0.6)), # 降低亮度
        #iaa.Fliplr(0.5),   #水平翻转
        #iaa.Flipud(0.5),   #垂直翻转
        #iaa.Rot90((1, 3), keep_size=False),
        #iaa.Cutout(nb_iterations=6, fill_mode="gaussian", fill_per_channel=True, size=0.1),  #添加色块

        iaa.FastSnowyLandscape(
            lightness_threshold=[128, 150],
